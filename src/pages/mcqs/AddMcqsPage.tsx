...
import { createMCQ } from "../../api/services/mcqService";
...
export default function AddMCQsPage() {
  ...
  const { data: pyqOptions = [], isLoading: loadingPyq } = useQuery({
    queryKey: ["pyqs"],
    queryFn: fetchAllPYQs,
  });

  const { mutate: submitMcq } = useMutation({
    ...
    <div className="add-mcq-page">
          ...
          renderInput={(params) => (
            <TextField
              ...
              InputProps={{
                ...
                endAdornment: (
                  <>
                    {loadingSubject && <CircularProgress size={20} />}
      ...
      <div className="item">
        ...
        <Autocomplete
          ...
          getOptionLabel={(opt: any) => opt.name}
          loading={loadingPyq}
          onChange={(e, val) => setFormData((prev) => ({ ...prev, pyq: val }))}
          renderInput={(params) => (
            <TextField
              {...params}
              ...
              InputProps={{
                ...
                endAdornment: (
                  <>
                    {loadingPyq && <CircularProgress size={20} />}
                    {params.InputProps.endAdornment}
                  </>
                ),
              }}
