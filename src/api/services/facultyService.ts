...
export const updateFaculty = async (id: string, data: Partial<Faculty>): Promise<Faculty> => {
    const res = await api.put<Faculty>(endpoints.faculty.update(id), data);
    return res.data;
};

// ✅ DELETE a faculty
export const deleteFaculty = async (id: string): Promise<{ message: string }> => {
    const res = await api.delete<{ message: string }>(endpoints.faculty.delete(id));
    return res.data;
};
